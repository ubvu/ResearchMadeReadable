...
#!/usr/bin/env python3
"""
Standalone test to replicate the exact parser logic.
"""
import bibtexparser
...
def clean_field(field_value: str) -> str:
...
def validate_bibtex_entry(entry) -> bool:
    ...
    result = all(field in entry and entry[field] for field in required_fields)
    print(f"    Validation details:")
    for field in required_fields:
        has_field = field in entry
        has_value = bool(entry.get(field, '')) if has_field else False
        print(f"      {field}: in_entry={has_field}, has_value={has_value}")
    print(f"    Overall valid: {result}")
    return result

def parse_bibtex_file(file_content: str):
    ...
    try:
        ...
        cleaned_content = preprocess_bibtex_content(file_content)
        print(f"   Cleaned content preview: {cleaned_content[:100]}...")
        
        # Create a fresh parser instance for each parse operation
        print("2. Creating parser...")
        parser = BibTexParser()
        parser.ignore_nonstandard_types = True
        parser.homogenize_fields = True
        parser.common_strings = True
        
        print("3. Parsing with bibtexparser...")
        bib_database = bibtexparser.loads(cleaned_content, parser=parser)
        print(f"   Found {len(bib_database.entries)} raw entries")
        
        papers = []
        
        for i, entry in enumerate(bib_database.entries):
            print(f"4. Processing entry {i+1}...")
            print(f"   Entry keys: {list(entry.keys())}")
            print(f"   Entry ID: {entry.get('ID', 'N/A')}")
            
            # Validate entry has minimum required fields
            print("5. Validating entry...")
            if not validate_bibtex_entry(entry):
                print("   ❌ Entry failed validation - skipping")
                continue
            
            print("   ✅ Entry passed validation")
            
            print("6. Extracting fields...")
            paper = {
                'title': clean_field(entry.get('title', '')),
                'authors': clean_field(entry.get('author', '')),
                'abstract': clean_field(entry.get('abstract', '')),
                'doi': entry.get('doi', ''),
            ...
            papers.append(paper)
            print("   ✅ Paper added to results")
        
        print(f"7. Final result: {len(papers)} papers processed")
        return papers
        
    except Exception as e:
        print(f"❌ Error parsing BibTeX file: {str(e)}")
        import traceback
        traceback.print_exc()
        return []

if __name__ == "__main__":
    print("Standalone BibTeX Parser Test")
    print("=" * 50)
    
    papers = parse_bibtex_file(test_bibtex)
